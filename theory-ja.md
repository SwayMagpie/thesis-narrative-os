# 00_overview.md
Thesis-Narrative OS — Theory Overview

## 0. Conceptual Lineage — Origin of Value Dimensions

Thesis‑Narrative OS は、**VBPM-Core（Value-Based Personality Model）** に由来する  
「価値基準（Value Dimensions）」の概念を継承している。

VBPM-Core では、人格を  
**価値基準 × 重み × 最適化**  
で定義するために、行動や状況を評価する軸として  
**Value Dimensions（価値基準／価値次元）** が導入された。

この価値次元は、探索（exploration）や意思決定を成立させるために  
必須の内部評価構造であり、外部化可能なベクトルとして扱われる。

Thesis‑Narrative OS は、この価値次元の概念を  
**キャラクターの内部状態（Value Vector）** として再構成し、  
物語生成における価値観の揺れ・情動・主観表現・イベント更新の  
中心構造として利用する。

本 OS における「価値次元」は、  
一般的な創作論の“価値観”とは異なり、  
**計算可能・更新可能・外部化可能な内部評価軸**として定義される。

---

## 1. Core Insight

本 OS の発端は以下の内部計算から始まった：

- 「事実ログを束ねれば物語は作れる」
- しかし本質は“事実ログだけではない”
- **価値観 × イベント × 情動 × 主観表現 × 世界整合**  
  を OS 的に結合することで、物語生成は完全に形式化できる

本理論は、物語を  
**価値関数の最適化・状態遷移ログのレンダリング**  
として扱う。

---

## 2. 全体フロー（OS 的プロセス）

### 1. Thesis（テーゼ）を立てる  
物語の価値関数の中心。  
OS 的には「最終的に到達する価値次元の方向ベクトル」。

例：自由とは何か／孤独は悪か／正しさは誰が決めるのか

---

### 2. テーゼを揺らす価値観を配置する  
物語を動かすキャラクターの生成。

- テーゼを体現する価値観  
- テーゼを揺らす価値観  
- テーゼに揺れる価値観  

OS 的対応：

- Character = 価値次元の重みベクトル  
- Conflict = ベクトル差分  
- Narrative = 差分が時間軸で収束していく過程  

---

### 3. テーゼの伝え方（問い or 主張）を決める  
物語の構造フォーマットを決定。

- 問い型 → 主人公が探索する構造  
- 主張型 → 主人公が証明する構造  

OS 的には「価値関数の最適化経路の形状」を決める工程。

---

### 4. Event（価値観更新ポイント）を設定する  
イベント = 主人公の価値ベクトルが更新される瞬間  
＝ 主観世界の差分が最大化される瞬間

---

## 3. Emotion Dynamics（情動力学）— OS の中核

### 情動は「キャラクターの内から湧き上がる内部現象」  
外部刺激ではなく、  
**価値観（Value Vector）とイベント（Event）の摩擦**  
から自然発生する。

### 定式化  


\[
Emotion = f(\Delta ValueVector \times EventIntensity)
\]



- ΔValueVector = 価値観の揺れ  
- EventIntensity = イベントの強度  
- EmotionStrength = 情動の強さ  

### Emotion Purity（情動純度）  
物語の「快」

- 揺れの大きさ  
- 揺れの方向の明確さ  
- 外部要因に汚染されていない内部発生性  



\[
Purity = |\Delta ValueVector|
\]



情動純度は **価値観の揺れの大きさとして観測される**。

---

## 4. Fact Log（事実ログ）  
イベント成立に必要な外界の状態遷移。

- 事実ログ = 外界の変化  
- 主観表現 = 内界の変化  
- イベント = その交点  

---

## 5. Subjective Expression（主観表現）  
価値観 × 事実ログ × 情動 によって生成される内界の状態遷移。

主観表現は「キャラクターが世界をどう経験したか」のログ。

---

## 6. World Consistency（整合性）  
外界（事実ログ）と内界（主観表現）が同時に成立する世界の構築。

現実的に起こりうる整合であるほど、  
世界モデルの確率分布に沿うため説得力が増す。

---

## 7. Narrative Rendering（物語レンダリング）  
以下を束ねて自然言語として出力する工程：

- 世界  
- 事実ログ  
- 主観表現  
- イベント  
- 情動  
- 整合性  

物語 = **価値観の変遷ログのレンダリング結果**

---

## 8. 数値モデルへの拡張可能性

本 OS は以下を数値化することで、  
物語の“必然性”を数学的に保証できる：

- 価値ベクトル  
- イベント更新量  
- 事実ログの重み  
- 情動強度・純度  
- 主観表現の深度  

これにより、  
**シミュレーション可能な物語生成 OS**  
として完成する。

---

## 9. 今後深掘りする論点

- テーゼの型  
- 価値観ベクトルの設計  
- イベント生成アルゴリズム  
- 情動生成モデルの精密化  
- 事実ログの構造化  
- 主観表現の生成ルール  
- 数値モデルとしての実装

## 利用者に必要な前提

Thesis‑Narrative OS は「誰でも使える物語テンプレート」ではありません。  
価値次元・状態遷移・情動力学・内部モデルといった  
抽象概念を扱える利用者を前提としています。

構造で物語を考えられる人向けの OS です。


# 01_thesis-model.md  
Thesis-Narrative OS — Thesis Model Specification

## 1. Thesis（テーゼ）の定義

テーゼとは、物語全体を貫く **中心的な価値関数（Value Function）** である。  
キャラクターの価値ベクトル、イベント更新、情動生成、主観表現など、  
OS 内のすべての計算はテーゼを基準として構造化される。

テーゼは以下の性質を持つ：

- **方向性（Directionality）**  
  価値次元空間における「向かうべき方向ベクトル」を定義する。

- **一貫性（Consistency）**  
  物語全体を通して保持される中心軸となる。

- **更新可能性（Updatability）**  
  キャラクターの価値ベクトルがテーゼに収束・反発する形で変化する。

- **評価基準（Evaluation Basis）**  
  イベントや主観表現の意味づけに影響を与える。

---

## 2. Thesis の役割

Thesis は OS 内で以下の役割を担う：

### 2.1 Narrative Trajectory（物語軌道）の決定  
物語がどの方向へ進むかを決定する。  
キャラクターの価値ベクトルは、テーゼに対して：

- 近づく  
- 離れる  
- 揺れる  

といった形で変化し、これが物語の軌道を形成する。

### 2.2 Conflict Structure（対立構造）の生成  
テーゼとキャラクターの価値ベクトルの差分が  
**対立（Conflict）** を生む。

### 2.3 Event Update の基準  
イベントは「価値ベクトルが更新される瞬間」であり、  
その更新方向はテーゼとの関係によって決まる。

### 2.4 Emotion Dynamics への影響  
価値ベクトルの揺れ（ΔValueVector）は、  
テーゼとの距離・方向によって意味づけられ、  
情動強度・純度の計算に影響する。

---

## 3. Thesis の型（Structural Types）

Thesis は OS 内で以下の構造型に分類される。  
これは「テーゼの立て方」ではなく、  
**テーゼが OS 内でどのように機能するか**を示す分類である。

### 3.1 Question Type（問い型）  
- 中心的な問いを設定する構造  
- キャラクターは問いに対して探索的に動く  
- Narrative Trajectory は「探索曲線」になる

例：  
- 自由とは何か  
- 正しさは誰が決めるのか

### 3.2 Assertion Type（主張型）  
- テーゼが明確な主張として存在する  
- キャラクターはその主張を証明・反証する方向に動く  
- Narrative Trajectory は「証明曲線」になる

例：  
- 孤独は悪ではない  
- 正義は個人の選択に宿る

### 3.3 Directional Type（方向型）  
- 抽象的な価値方向を示す  
- キャラクターはその方向に収束・逸脱する形で動く  
- Narrative Trajectory は「収束曲線」になる

例：  
- 自律へ向かう  
- 調和へ向かう

---

## 4. Thesis と Value Dimensions の関係

Thesis は **価値次元空間における方向ベクトル** として定義される。  
これにより、以下の OS 内計算が可能になる：

- キャラクターの価値ベクトルとの距離計算  
- 価値ベクトル更新方向の決定  
- 情動純度の意味づけ  
- 主観表現の方向性の決定  

Thesis は「価値次元のどの方向が重要か」を示す  
**重み付けの基準点**として機能する。

---

## 5. Thesis と Event の関係

イベントは「価値ベクトルが更新される瞬間」であり、  
その更新方向は Thesis によって決まる。

- テーゼに近づく更新  
- テーゼから離れる更新  
- テーゼに対して揺れる更新  

これらの差分が Narrative Trajectory を形成する。

---

## 6. Thesis と Emotion Dynamics の関係

Emotion Dynamics の計算式：



\[
Emotion = f(\Delta ValueVector \times EventIntensity)
\]



ここで ΔValueVector の意味づけは Thesis に依存する。

- テーゼに近づく揺れ  
- テーゼから離れる揺れ  
- テーゼに対して方向が曖昧な揺れ  

これらが情動強度・純度の解釈に影響する。

---

## 7. Thesis の構造的要件（Structural Requirements）

Thesis は以下の要件を満たす必要がある：

- **方向性があること**  
  価値次元空間でベクトルとして扱える。

- **一貫性があること**  
  物語全体を通して中心軸として機能する。

- **抽象度が適切であること**  
  価値ベクトルの更新を許容するレベルの抽象性を持つ。

- **更新可能性があること**  
  キャラクターの変化を説明できる。

- **価値次元と整合すること**  
  価値ベクトルとの距離・方向が計算可能である。

---

## 8. 本章が提供しないもの（Non-Scope）

本章は **OS の構造のみ**を扱う。  
以下は本章の対象外であり、外部化しない：

- テーゼをどう思いつくか  
- テーゼをどう選ぶか  
- テーゼをどう評価するか  
- テーゼ生成の思考プロセス  
- 個人固有の価値観・感性  
- 内部 OS（主観世界モデル）の詳細  

これらは利用者の内部モデルに依存するため、  
OS の仕様書には含めない。


# 02_value-vector.md  
Thesis-Narrative OS — Value Vector Specification

## 1. Value Vector の定義

Value Vector は、キャラクターの **内部状態を表す多次元ベクトル** である。  
各次元は Value Dimension に対応し、キャラクターがどの方向へ揺れ、  
どの方向へ更新されるかを定義する。

Value Vector は以下の役割を持つ：

- テーゼとの距離・方向の計算  
- イベント更新の対象  
- 情動生成の基礎データ  
- 主観表現の源泉  

キャラクターとは、OS 的には **Value Vector の集合体** である。

---

## 2. キャラクター配置の基本原理

テーゼが決まった時点で、  
キャラクター配置は **「テーゼに対してどう揺れるか」** の問題に還元される。

OS 的には、キャラクターは以下の 2 系統に分類される：

- **テーゼを揺らすもの（回答を持つ者）**  
- **テーゼに揺れるもの（回答を持たない者）**

この分類はキャラクターの性格や役割ではなく、  
**価値ベクトルの構造的分類**である。

---

## 3. テーゼを揺らすもの（回答を持つ者）

テーゼに対して **自分なりの“回答”を持つキャラクター**。  
その回答が肯定か反発かは問わない。

### 3.1 テーゼの肯定者・体現者  
- テーゼと同方向の価値ベクトルを持つ  
- テーゼの方向性を強化する  
- Narrative Trajectory の「収束側」を形成する

### 3.2 テーゼの反発者  
- テーゼと逆方向の価値ベクトルを持つ  
- テーゼを揺らし、対立構造を生む  
- Narrative Trajectory の「反発側」を形成する

これらは OS 内で **テーゼの揺れ幅を作るための構造要素** である。

---

## 4. テーゼに揺れるもの（回答を持たない者）

テーゼに対して **自分の“回答”を持たないキャラクター**。  
価値ベクトルが未確定、または方向性が曖昧な状態にある。

特徴：

- テーゼの方向性に対して揺れ動く  
- イベントによって価値ベクトルが大きく更新される  
- Narrative Trajectory の「変化の中心」になりやすい

OS 的には、  
**ΔValueVector（揺れ）が最も大きく発生する構造要素** である。

---

## 5. キャラクター数の要件

Thesis-Narrative OS は、  
**キャラクター数に依存しない構造** を持つ。

極論として：

- **1キャラクターのみ**  
- 他はすべて **イベントとして外部化**

という構成でも物語は成立する。

理由：

- 価値ベクトルの更新  
- テーゼとの距離変化  
- 情動生成  
- 主観表現  
- 世界整合  

これらが OS 内で完結しているため、  
キャラクター数は Narrative Rendering の問題ではなく、  
**Value Vector の更新問題**に還元されるからである。

---

## 6. 本章が提供しないもの（Non-Scope）

本章は **Value Vector の構造のみ** を扱う。  
以下は対象外とする：

- キャラクター性格の作り方  
- キャラクターの背景設定  
- キャラクターの感情生成プロセス  
- キャラクターの思想形成  
- キャラクターの“回答”をどう思いつくか  
- キャラクターの創作手法全般  

これらは利用者の内部モデルに依存するため、  
OS の仕様書には含めない。


# 03_event-update.md  
Thesis-Narrative OS — Event Update Specification

## 1. Event Update の定義

Event Update とは、  
キャラクターの **Value Vector が更新される瞬間** を指す。

OS 内では、イベントは以下の 3 要素の交点として成立する：

- **外部状態の変化（Fact Log）**  
- **内部状態の変化（Value Vector Update）**  
- **テーゼとの距離・方向の変化（ΔValueVector）**

イベントは「出来事」ではなく、  
**価値ベクトルが変化したという計算上の事実**である。

---

## 2. イベントはテーゼと主人公から構造的に決まる

Thesis-Narrative OS では、  
イベントは創作者が“考える”ものではなく、  
**テーゼと主人公の関係から必然的に導かれる更新点**である。

### 2.1 テーゼが決まる  
テーゼは物語の中心となる **方向ベクトル** を定義する。

### 2.2 Thesis Type（Question / Assertion）が決まる  
- Question Type → 主人公が揺れやすい構造  
- Assertion Type → 主人公が揺らしやすい構造  

これにより、Narrative Trajectory の **更新軌道の形** が決まる。

### 2.3 主人公が決まる  
主人公とは、  
**テーゼとの関係において、物語上もっとも大きな変化・影響を担う Value Vector**  
を持つキャラクターである。

この「変化・影響」は次のいずれの形でもよい：

- **自身の Value Vector が大きく揺れる**（テーゼに揺れる）  
- **テーゼ側の意味づけや位置づけを揺らす**（テーゼを揺らす）

主人公は、テーゼとの関係における  
**主要な更新の中心** として Narrative Trajectory を形成する。

### 2.4 主人公とテーゼの差分からイベントが決まる  
主人公の Value Vector とテーゼの方向ベクトルの差分を取ると、  
**必要な更新点（マイルストーン）が自動的に決まる**。

OS 的には：



\[
Event = f(\Delta ValueVector)
\]



つまり、  
**イベントは“必要な更新点”として構造的に存在する**。

---

## 3. Event Update の構造

Event Update は以下の 3 層で構成される：

### 3.1 External Layer（外部変化）  
Fact Log によって記述される外部状態の変化。

### 3.2 Internal Layer（内部変化）  
Value Vector の更新量（ΔValueVector）。

### 3.3 Thesis Layer（意味づけ）  
更新がテーゼに対して：

- 近づく  
- 離れる  
- 揺れる  
- 揺らす  

のどれに該当するかを決定する。

---

## 4. Event Update の種類（構造分類）

イベントは、更新方向によって以下の 4 種に分類される：

### 4.1 Convergence Event（収束イベント）  
Value Vector がテーゼ方向に更新される。

### 4.2 Divergence Event（乖離イベント）  
Value Vector がテーゼから離れる方向に更新される。

### 4.3 Oscillation Event（揺れイベント）  
方向が定まらず、揺れ幅が大きい更新。

### 4.4 Impact Event（揺らしイベント）  
主人公側の更新ではなく、  
**テーゼ側の意味づけが揺らされる更新**。

（Assertion 型で特に発生しやすい）

---

## 5. Event Sequence（イベント列）の決定

イベント列は **マイルストーン（必然）** と **経路（自由）** に分かれる。

### 5.1 マイルストーン（必然的に必要な更新点）  
主人公とテーゼの差分から自動的に決まる。

- 更新すべき方向  
- 更新すべき量  
- 更新すべき順序（部分的）

これらは OS 内で構造的に固定される。

### 5.2 経路（自由に選べる部分）  
マイルストーン間をどう繋ぐかは自由。

- どの Fact Log を使うか  
- どのキャラクターを関与させるか  
- どのような演出にするか  

OS が規定するのは **更新点** であり、  
**演出や順番は OS の外側**である。

---

## 6. Event Update の要件（Structural Requirements）

イベントは以下の要件を満たす必要がある：

- Value Vector が更新されている  
- 更新方向がテーゼと比較可能である  
- Fact Log によって外部状態が説明可能である  
- 主観表現が生成可能である  
- 世界整合性を保てる  

これらを満たさないものは OS 内では「イベント」として扱われない。

---

## 7. 本章が提供しないもの（Non-Scope）

本章は Event Update の **構造のみ** を扱う。  
以下は対象外とする：

- イベントの内容（出来事）  
- どのような演出にするか  
- どのキャラクターを関与させるか  
- どのようにイベントを思いつくか  
- プロットの作り方  
- 物語のテンプレート  

これらは創作者の内部モデルに依存するため、  
OS の仕様書には含めない。


# 04_emotion-dynamics.md  
Thesis-Narrative OS — Emotion Dynamics Specification

## 1. Emotion Dynamics の定義

Emotion Dynamics とは、  
**Value Vector の変化量（ΔValueVector）と EventIntensity に基づいて  
情動の強度と純度を計算する OS 内部の構造**である。

情動は OS 内では「内部状態の変化として観測される量」であり、  
外部刺激そのものではなく、  
**Value Vector の更新結果として定義される。**

---

## 2. Emotion Dynamics の基本式

Emotion Dynamics は以下の式で定義される：



\[
Emotion = f(\Delta ValueVector \times EventIntensity)
\]



ここで：

- **ΔValueVector**  
  価値ベクトルの変化量（揺れ）

- **EventIntensity**  
  イベントの強度（Fact Log による外部変化の大きさ）

- **EmotionStrength**  
  情動の強さ（出力）

Emotion は、  
**内部状態の変化 × 外部変化の強度**  
として計算される。

---

## 3. Emotion Purity（情動純度）

Emotion Purity は、  
**Value Vector の揺れの大きさとして観測される量**である。

OS 内では以下のように定義される：



\[
Purity = |\Delta ValueVector|
\]



Emotion Purity は次の要素を含む：

- **揺れの大きさ**  
  どれだけ価値ベクトルが変化したか

- **揺れの方向の明確さ**  
  更新方向がどれだけ一貫しているか

- **内部発生性**  
  更新が内部状態の変化として説明可能であること

Emotion Purity は、  
**物語内部での「快」や「意味の強度」** を測定するための  
構造的な指標として扱われる。

---

## 4. ΔValueVector の設計とイベント強度

Emotion Dynamics を成立させるためには、  
**どの Value Dimension にどれだけの Δ（揺れ）を発生させるか**  
を決定する必要がある。

これは OS の外側にある創作者の判断に依存するが、  
OS としては以下の構造を提供する：

### 4.1 ΔValueVector の構造的要件  
- どの次元に変化が発生したか  
- 変化量がどれだけか  
- 変化方向がテーゼと比較可能であること  
- 更新が Fact Log と整合していること  

### 4.2 EventIntensity の構造的要件  
- 外部変化の大きさを表すスカラー値  
- ΔValueVector と組み合わせて EmotionStrength を決定する  
- Fact Log によって説明可能であること  

Emotion Dynamics は、  
**内部変化（ΔValueVector）と外部変化（EventIntensity）の  
両方が揃ったときに成立する構造**である。

---

## 5. Emotion Dynamics の役割

Emotion Dynamics は OS 内で以下の役割を担う：

- **Narrative Trajectory の強度を決定する**  
  揺れの大きさが物語の推進力になる。

- **Subjective Expression の基礎データとなる**  
  主観表現は EmotionStrength と Purity をもとに生成される。

- **Fact Log の設計難易度に影響する**  
  ΔValueVector の大きさに応じて、  
  必要な外部変化（Fact Log）の設計が変わる。

Emotion Dynamics は、  
**物語の内部計算を支える中心的なエンジン**である。

---

## 6. 本章が提供しないもの（Non-Scope）

本章は Emotion Dynamics の **構造のみ** を扱う。  
以下は対象外とする：

- 情動をどう「感じさせる」か  
- どのような情動を「演出」するか  
- ΔValueVector をどう「決める」か  
- EventIntensity をどう「設定する」か  
- 物語の快をどう「作る」か  
- 創作者の価値観・感性・判断基準  

これらは創作者の内部モデルに依存するため、  
OS の仕様書には含めない。

# 05_fact-log.md  
Thesis-Narrative OS — Fact Log Specification

## 1. Fact Log（事実ログ）の定義

Fact Log とは、  
**外界における状態遷移を記録する構造データ**である。

OS 内では、以下の 3 要素が明確に分離される：

- **Fact Log（外界の変化）**  
- **Subjective Expression（内界の変化）**  
- **Event Update（外界と内界の交点）**

Fact Log は「出来事の描写」ではなく、  
**イベント成立に必要な外界の変化を構造的に記述したもの**である。

---

## 2. Fact Log の役割

Fact Log は OS 内で以下の役割を担う：

- **Event Update の外部側の根拠を提供する**  
- **Value Vector の更新を外界の変化と整合させる**  
- **World Consistency（世界整合性）を保証する**  
- **マイルストーン間の“道中”を構造化する**

Fact Log は、  
**イベントを成立させるための外界の因果関係**を記述するための  
最小限の構造である。

---

## 3. Fact Log と Event の関係

OS 内では、以下の関係が成立する：

- **Fact Log = 外界の変化**  
- **Subjective Expression = 内界の変化**  
- **Event = その交点（Value Vector Update）**

イベントは Fact Log がなければ成立しない。  
Fact Log はイベントの「外部側の証拠」である。

---

## 4. Fact Log の構造

Fact Log は以下の要素で構成される：

### 4.1 External State Before  
イベント前の外界状態。

### 4.2 External State After  
イベント後の外界状態。

### 4.3 State Transition  
外界がどのように変化したかを示す差分。

### 4.4 Causal Link  
その外界変化が、  
Value Vector の更新（ΔValueVector）と  
整合するための因果的説明。

Fact Log は「何が起きたか」ではなく、  
**外界がどう変化したか**を記述する。

---

## 5. マイルストーン間の Fact Log

イベント（マイルストーン）が決まったら、  
**前回のイベントから今回のイベントに至るまでの外界の変化**を  
Fact Log として書き出す。

OS 的には：

- マイルストーン（更新点）は構造的に決まる  
- その間をどう繋ぐか（Fact Log）は自由  
- ただし World Consistency を満たす必要がある

Fact Log は **マイルストーン間の“道中”を構造化するデータ**である。

---

## 6. Fact Log と World Consistency

Fact Log は World Consistency に強く依存する。

World Consistency とは：

- 外界の変化が矛盾していない  
- 価値ベクトルの更新と整合している  
- 主観表現と矛盾しない  
- 物語世界のルールに反しない  

Fact Log は、  
**世界整合性を保ちながら Value Vector を更新するための  
外界側の構造的要件**である。

---

## 7. Fact Log の設計難易度

Fact Log の設計は OS 内で最もデリケートな工程のひとつである。

理由：

- ΔValueVector の大きさに応じて必要な外界変化が変わる  
- EventIntensity と整合させる必要がある  
- World Consistency を維持しなければならない  
- 主観表現との整合も必要  

OS としては構造だけ提供し、  
**どの次元にどれだけの変化を置くかは創作者の判断**に委ねる。

---

## 8. 本章が提供しないもの（Non-Scope）

本章は Fact Log の **構造のみ** を扱う。  
以下は対象外とする：

- どのような出来事を描くか  
- どのように Fact Log を思いつくか  
- どの程度の外界変化を設定するか  
- 物語の演出方法  
- 世界観の作り方  
- 創作者の価値観・判断基準  

これらは創作者の内部モデルに依存するため、  
OS の仕様書には含めない。


# 06_subjective-expression.md  
Thesis-Narrative OS — Subjective Expression Specification

## 1. Subjective Expression（主観表現）の定義

Subjective Expression とは、  
**キャラクターが世界をどのように経験したかを記録する  
内界の状態遷移ログ**である。

OS 内では、主観表現は以下の 3 要素の組み合わせによって生成される：

- **Value Vector（価値観）**  
- **Fact Log（外界の変化）**  
- **Emotion Dynamics（情動）**

主観表現は「キャラクターの感情」ではなく、  
**内部計算の結果として得られる“経験のログ”**である。

---

## 2. 主観表現の役割

Subjective Expression は OS 内で以下の役割を担う：

- **イベントの内界側の根拠を提供する**  
- **Value Vector の更新がどのように経験されたかを記述する**  
- **Emotion Dynamics の出力を物語内部で解釈可能にする**  
- **外界（Fact Log）と内界（Emotion）の橋渡しを行う**

主観表現は、  
**外界の変化がキャラクター内部でどのように意味づけられたか**  
を記述する構造である。

---

## 3. 主観表現は“計算結果のログ”である

主観表現は創作的に「考える」ものではなく、  
**既に出そろっているデータから計算される値**である。

具体的には：



\[
SubjectiveExpression = g(ValueVector, FactLog, Emotion)
\]



主観表現は、  
**計算式の途中式を外部化したもの**であり、  
その難易度は前工程の設計値に依存する。

---

## 4. 主観表現の構造

Subjective Expression は以下の要素で構成される：

### 4.1 Internal State Before  
イベント前の内部状態（Value Vector）。

### 4.2 Internal State After  
イベント後の内部状態（更新後の Value Vector）。

### 4.3 Interpretation of External Change  
Fact Log に記述された外界の変化が  
内部でどのように意味づけられたか。

### 4.4 Emotional Response  
Emotion Dynamics によって計算された  
EmotionStrength と EmotionPurity。

### 4.5 Integrated Experience  
上記の要素を統合した  
「キャラクターが世界をどう経験したか」のログ。

主観表現は、  
**内部状態の変化を説明するための構造化データ**である。

---

## 5. 主観表現とイベントの関係

OS 内では以下の関係が成立する：

- **Fact Log = 外界の変化**  
- **Subjective Expression = 内界の変化**  
- **Event = その交点（Value Vector Update）**

主観表現は、  
**イベントの内界側の証拠**として機能する。

---

## 6. 主観表現の設計難易度

主観表現の難易度は、  
**前工程の設計値に強く依存する**。

- ΔValueVector の大きさ  
- EventIntensity の設定  
- Fact Log の整合性  
- Emotion Dynamics の出力  

これらが複雑であればあるほど、  
主観表現の設計も複雑になる。

OS としては構造だけ提供し、  
**どのように経験させるかは創作者の判断**に委ねる。

---

## 7. 本章が提供しないもの（Non-Scope）

本章は Subjective Expression の **構造のみ** を扱う。  
以下は対象外とする：

- キャラクターの感情表現の作り方  
- どのように主観を描写するか  
- どのような語り口にするか  
- どのように“経験”を演出するか  
- どのような感情を持たせるか  
- 創作者の価値観・判断基準  

これらは創作者の内部モデルに依存するため、  
OS の仕様書には含めない。

# 07_world-consistency.md  
Thesis-Narrative OS — World Consistency Specification

## 1. World Consistency（世界整合性）の定義

World Consistency とは、  
**外界（Fact Log）と内界（Subjective Expression）が  
同時に成立可能である世界モデルの整合条件**である。

OS 内では、世界整合性は  
Value Vector の更新（Event Update）が破綻なく成立するための  
**必要条件**として扱われる。

---

## 2. World Consistency の役割

World Consistency は OS 内で以下の役割を担う：

- **Fact Log（外界の変化）が矛盾しないことを保証する**  
- **Subjective Expression（内界の変化）が外界と整合することを保証する**  
- **Event Update が成立可能である世界を定義する**  
- **Narrative Trajectory（価値ベクトルの軌道）を支える舞台を構築する**

世界整合性は「世界観」ではなく、  
**更新が破綻しないための制約条件**である。

---

## 3. World Consistency の必要条件

世界整合性を満たすためには、  
以下の条件が必要となる：

### 3.1 外界の整合性  
- Fact Log の状態遷移が矛盾していない  
- 世界のルールに反しない  
- 物理・社会・文化的な因果が破綻していない  

### 3.2 内界の整合性  
- Subjective Expression が Fact Log と矛盾しない  
- Emotion Dynamics の出力と Value Vector の更新が一致している  

### 3.3 更新の整合性  
- Event Update が外界と内界の両方から説明可能である  
- ΔValueVector が Fact Log と Emotion によって正当化される  

これらが満たされない場合、  
OS 内では物語が「破綻」とみなされる。

---

## 4. 世界モデルと確率分布

World Consistency は、  
**世界モデルの確率分布に沿うほど説得力が増す**  
という性質を持つ。

これは OS 的には：

- 世界モデルの確率分布に沿う → 更新が自然に見える  
- 確率分布から外れる → 更新が不自然に見える  

という構造を意味する。

---

## 5. World Building（世界観構築）との関係

World Consistency は **必要条件**であり、  
World Building は **自由条件**である。

### 5.1 必要条件（OS が規定する）  
- Fact Log と Subjective Expression が同時に成立すること  
- Event Update が破綻しないこと  
- 世界モデルの確率分布に反しないこと  

### 5.2 自由条件（創作者が自由に拡張できる）  
- 世界観の厚み  
- 設定の深さ  
- 文化・歴史・技術体系  
- 物語を面白くするための追加要素  

OS は必要条件のみを規定し、  
自由条件は創作者に委ねる。

---

## 6. World Consistency の導入タイミング

World Consistency は、  
以下の構造が揃った後に初めて定義可能となる：

- Value Vector  
- Event Update  
- Fact Log  
- Subjective Expression  

これらが揃って初めて、  
**「外界と内界が同時に成立する世界」**  
を設計できる。

このため、  
World Consistency は OS のこの段階で導入される。

---

## 7. 本章が提供しないもの（Non-Scope）

本章は World Consistency の **必要条件のみ** を扱う。  
以下は対象外とする：

- 世界観の作り方  
- 設定の厚みの出し方  
- 物語を面白くするための演出  
- 世界観の創作技法  
- 創作者の価値観・判断基準  

これらは創作者の内部モデルに依存するため、  
OS の仕様書には含めない。


# 08_narrative-rendering.md  
Thesis-Narrative OS — Narrative Rendering Specification

## 1. Narrative Rendering（物語レンダリング）の定義

Narrative Rendering とは、  
これまでに生成された以下の構造データを束ね、  
**自然言語として出力する最終工程**である：

- World Model（世界）  
- Fact Log（外界の変化）  
- Subjective Expression（内界の変化）  
- Event Update（価値ベクトルの更新）  
- Emotion Dynamics（情動）  
- World Consistency（整合性）

Narrative Rendering は「創作」ではなく、  
**内部計算結果の自然言語化**である。

---

## 2. Narrative Rendering の役割

Narrative Rendering は OS 内で以下の役割を担う：

- **価値観の変遷ログを自然言語として可視化する**  
- **Fact Log と Subjective Expression を統合して物語として提示する**  
- **Event Update の連続を Narrative Trajectory として表現する**  
- **Emotion Dynamics を読者が理解可能な形に変換する**

Narrative Rendering の本質は：



\[
物語 = 価値観の変遷ログのレンダリング結果
\]



である。

---

## 3. Narrative Rendering の構造

Narrative Rendering は以下の要素を統合する：

### 3.1 External Layer（外界）  
- Fact Log  
- World Model  
- World Consistency  

### 3.2 Internal Layer（内界）  
- Value Vector  
- Subjective Expression  
- Emotion Dynamics  

### 3.3 Update Layer（更新）  
- Event Update  
- Narrative Trajectory（価値ベクトルの軌道）

これらを束ねて自然言語として出力する。

---

## 4. Narrative Rendering の計算モデル

Narrative Rendering は、  
以下のような統合関数として定義される：



\[
Narrative = h(World, FactLog, SubjectiveExpression, Event, Emotion, Consistency)
\]



ここで重要なのは：

- **Narrative は新しい情報を生成しない**  
- **Narrative は既存の内部計算結果を自然言語に変換するだけ**

という点である。

---

## 5. Narrative Rendering と LLM

Narrative Rendering は、  
**LLM に委譲可能な工程**である。

理由：

- ここまでの工程で必要なデータはすべて揃っている  
- Narrative Rendering は「加工」であり「創作」ではない  
- LLM は自然言語化に特化している  
- OS の目的は「LLM に渡すための構造化データを作ること」

そのため、  
**OS の最終工程は LLM に任せる設計が自然**である。

---

## 6. Narrative Rendering の自由度

Narrative Rendering は OS の外側にある  
**創作者の自由領域**である。

自由に決めてよいもの：

- 文体  
- 語り口  
- 視点（1人称・3人称など）  
- 時制  
- 文学的表現  
- 情緒的演出  
- 物語のテンポ  
- どこまで描写するか  

OS が規定するのは **構造**であり、  
**表現方法は創作者の自由**である。

---

## 7. Narrative Rendering の必要条件

Narrative Rendering が成立するためには：

- Fact Log と Subjective Expression が整合している  
- Event Update が連続している  
- Emotion Dynamics が反映されている  
- World Consistency が保たれている  

これらの条件が必要となる。

これらが満たされない場合、  
Narrative Rendering は破綻する。

---

## 8. 本章が提供しないもの（Non-Scope）

本章は Narrative Rendering の **構造のみ** を扱う。  
以下は対象外とする：

- 文体の選び方  
- 物語の書き方  
- どのように読者を感動させるか  
- どのように描写するか  
- どのように語り口を選ぶか  
- 創作者の価値観・判断基準  

これらは創作者の内部モデルに依存するため、  
OS の仕様書には含めない。

# 09_numeric-extension.md  
Thesis-Narrative OS — Numerical Extension Possibilities

## 1. 数値モデルへの拡張意義

本 OS は、  
**価値観・イベント・情動・主観・世界整合性**  
といった物語生成の構造を定義する。

これらを数値化することで、  
物語の「必然性」を数学的に保証し、  
**シミュレーション可能な物語生成 OS** へと拡張できる。

---

## 2. 数値化可能な構造要素

以下の OS 構造は数値モデルに変換可能である：

### 2.1 Value Vector（価値ベクトル）  
- 各 Value Dimension を数値軸として扱う  
- キャラクターの内部状態をベクトルとして表現可能  

### 2.2 Event Update（イベント更新量）  
- ΔValueVector を数値として扱う  
- 更新方向・更新量を数学的に計算可能  

### 2.3 Fact Log（事実ログの重み）  
- 外界変化の強度をスカラー値として扱う  
- EventIntensity と連動させることで因果の強度を表現  

### 2.4 Emotion Dynamics（情動強度・純度）  
- EmotionStrength = f(ΔValueVector × EventIntensity)  
- EmotionPurity = |ΔValueVector|  
- 情動を完全に数値モデル化可能  

### 2.5 Subjective Expression（主観表現の深度）  
- 内界の変化量をスカラーまたはベクトルとして扱う  
- 外界との整合度を数値で測定可能  

---

## 3. 数値モデル化によるメリット

### 3.1 物語の必然性の保証  
- Value Vector の軌道が数学的に決まる  
- Event Update が確率的に妥当か判定可能  
- World Consistency を数値的に評価可能  

### 3.2 シミュレーション可能な物語生成  
- キャラクターの行動を Value Vector の更新として計算  
- イベント列を自動生成  
- 情動の変化を自動計算  
- 主観表現を自動生成  

### 3.3 LLM とのハイブリッド化  
- OS が数値データを生成  
- LLM が Narrative Rendering を担当  
- 完全自動の物語生成パイプラインが成立  

---

## 4. 数値モデル化の方向性

数値モデル化は以下の方向で拡張可能である：

- **確率モデル**（Value Vector の遷移確率）  
- **最適化モデル**（物語の快を最大化する軌道）  
- **強化学習モデル**（キャラクターの行動選択）  
- **物理シミュレーション的世界モデル**  
- **感情ダイナミクスの微分方程式化**  

これらは OS の外側にある「未来の拡張」であり、  
本編の必要条件ではない。

---

## 5. 本章の位置づけ（Core ではなく Extension）

本章は OS の **必須構造ではなく、拡張可能性の提示**である。

- 物語は 01〜08 で完成する  
- 数値モデル化は「未来の選択肢」  
- 必要条件ではなく、発展可能性  

本 OS は、  
**構造 → 計算 → レンダリング**  
という流れを持つため、  
数値モデル化との相性が非常に良い。

本章はその可能性を示すための付録である。

# 10_character-impression.md  
Thesis-Narrative OS — Numerical Extension Possibilities
キャラクター初登場時の印象形成を数式化した OS 下位モジュール

キャラクターの「初登場時の記憶定着」を  
**印象ベクトル \( I \)** と **閾値 \( \theta \)** によって定式化する。  
これはキャラクター生成の OS 的基盤となる。

---

## 1 印象強度 \( I \) の定義（ベクトルモデル）

キャラクターの印象は、  
6つの特徴ベクトル \( f_k \) の重み付き和で表される。



\[
I = \sum_{k=1}^{6} w_k \cdot f_k
\]



- \( f_k \)：特徴の強度（0〜1）
- \( w_k \)：特徴の重要度（重み）

---

## 2 印象を構成する6つの特徴ベクトル

| 次元 | 内容 | 記号 |
|------|------|------|
| Scene | シーン類型との結びつき | \( f_{\text{scene}} \) |
| Value | 価値観 → 行動の反映 | \( f_{\text{value}} \) |
| Reaction | 反応パターンの一貫性 | \( f_{\text{reaction}} \) |
| Contrast | 他キャラとの対比 | \( f_{\text{contrast}} \) |
| Role | 役割の明確さ | \( f_{\text{role}} \) |
| First | 初登場の強度 | \( f_{\text{first}} \) |

これら6次元は **人間の認知構造に依存する普遍的特徴空間** であり、  
ジャンルによって変化しない。

---

## 3 印象閾値 \( \theta \) の定義

読者がキャラを「名前で識別」できる最低ライン。



\[
I \ge \theta
\]



- 読者の認知負荷に依存  
- 娯楽作品では **作品側が閾値を超えさせる必要がある**

---

## 4 新規キャラ登場ルール（OS 仕様）

初登場シーンで以下を満たす必要がある：



\[
w_{\text{scene}} f_{\text{scene}}
+ w_{\text{value}} f_{\text{value}}
+ w_{\text{reaction}} f_{\text{reaction}}
+ w_{\text{contrast}} f_{\text{contrast}}
+ w_{\text{role}} f_{\text{role}}
+ w_{\text{first}} f_{\text{first}}
\ge \theta
\]



これが **新規キャラ登場の OS ルール**。

---

## 5 OS 的アルゴリズム化

### ● ルール1：初登場で最低1つは強い特徴を出す



\[
\max(f_k) \ge \alpha
\]



- \( \alpha \)：強い印象の最低値（例：0.7）
- **尖った特徴が1つ必要**

---

### ● ルール2：弱い特徴を積んでも意味がない



\[
\sum_{k=1}^{6} f_k < \theta \quad \Rightarrow \quad 記憶されない
\]



- 薄い特徴の積み重ねは無効  
- 「強い一点突破」が必須

---

### ● ルール3：名前は印象が閾値を超えた後に定着する



\[
\text{NameBind} =
\begin{cases}
1 & (I \ge \theta) \\
0 & (I < \theta)
\end{cases}
\]



- 名前は「印象のハッシュキー」  
- 印象が弱いと名前が結びつかない

---

## 6 新規キャラ登場アルゴリズム（最終形）

1. **初登場シーンを設計する**  
2. **6つの特徴ベクトル \( f_k \) を評価する**  
3. **重み付き和で印象強度 \( I \) を計算する**  
4. **閾値 \( \theta \) と比較する**  
5.  
   - \( I \ge \theta \)：読者が名前を記憶  
   - \( I < \theta \)：読者はキャラを識別できない

---

## 7 feature（印象ベクトルの次元）はジャンル共通の普遍構造

理由：

- 人間がキャラを覚える仕組みは **ジャンルで変わらない**
- 認知心理学的に普遍的な6次元

ジャンルが変えるのは **刺激の種類** だけ。

### ジャンル別の刺激例
- ホラー → 恐怖刺激  
- ラブコメ → 親密性・距離感  
- バトル → 行動・価値観の衝突  
- ミステリ → 推理行動・論理性  

**feature 次元は普遍、刺激はジャンル依存。**


